...
describe("KBStore", () => {
  ...
  describe("list + filters", () => {
    beforeEach(() => {
      store.create({ type: "lesson", title: "Lesson 1", content: "L1", tags: ["deploy"], priority: "high" });
      store.create({ type: "gotcha", title: "Gotcha 1", content: "G1", tags: ["deploy", "auth"] });
      store.create({ type: "convention", title: "Convention 1", content: "C1", tags: ["code"] });
      store.create({ type: "lesson", title: "Lesson 2", content: "L2 about authentication", tags: ["auth"], priority: "critical" });
    });
    ...
    it("filters by type", () => {
      const lessons = store.list({ type: "lesson" });
      expect(lessons.length).toBe(2);
    });

    it("filters by tag", () => {
      const authEntries = store.list({ tag: "auth" });
      expect(authEntries.length).toBe(2);
    });

    it("filters by priority", () => {
      const critical = store.list({ priority: "critical" });
      expect(critical.length).toBe(1);
      expect(critical[0].title).toBe("Lesson 2");
    });

    it("full-text search", () => {
      const results = store.list({ search: "authentication" });
    ...
    it("excludes expired entries by default", () => {
      ...
      const entries = store.list({ includeExpired: true });
  ...
  describe("briefing", () => {
    it("generates markdown briefing", () => {
      store.create({ type: "lesson", title: "Test Lesson", content: "Content here", tags: ["test"] });
      store.create({ type: "gotcha", title: "Test Gotcha", content: "Watch out!", priority: "critical" });
    ...
    it("filters by tags", () => {
      store.create({ type: "lesson", title: "Deploy Lesson", content: "D", tags: ["deploy"] });
      store.create({ type: "lesson", title: "Auth Lesson", content: "A", tags: ["auth"] });
  ...
  describe("stats", () => {
    it("returns correct statistics", () => {
      store.create({ type: "lesson", title: "L1", content: "C", tags: ["a", "b"], priority: "high" });
      store.create({ type: "gotcha", title: "G1", content: "C", tags: ["a"] });
  ...
  describe("persistence", () => {
    it("survives reload", () => {
      store.create({ type: "lesson", title: "Persistent", content: "Survives" });
      store.flush();
